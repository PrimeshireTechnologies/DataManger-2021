                        ...
                        url: '/Upload/PatientView', type: 'POST', data: JSON.stringify(patientdataview), dataType: 'json', contentType: "application/json", success: function (response) {
                            ...
                            if (response !== "") {
                                ...
                                for (var i = 0; i < response.length; i++) {
                                    ...
                                    document.getElementById("col8").style.fontSize   = "10px";
                                    document.getElementById("col8").style.color      = "red";
            ...
            url: '/Home/PatientView', type: 'POST', data: JSON.stringify(patientdataview), dataType: 'json', contentType: "application/json", success: function (response) {
                ...
                if (response !== "" && counterfetch === "0") {
                    ...
                    for (var i = 0; i < response.length; i++) {
                        ...
                        //patientview.push({ ClientID: padding_right(response[i].ClientID, '\u00A0', 5), FirstName: padding_right(response[i].FirstName, '\u00A0', 5), LastName: padding_right(response[i].LastName, '\u00A0', 5), DOB: padding_right(formatDate(response[i].DOB), '\u00A0', 5), Gender: padding_right(response[i].Gender, '\u00A0', 5), FullStreetAddress: padding_right(response[i].FullStreetAddress, '\u00A0', 5), City: padding_right(response[i].City, '\u00A0', 5), State: padding_right(response[i].State, '\u00A0', 5), Email_Address: padding_right(response[i].Email_Address, '\u00A0', 5) });                                                                        
                        patientview.push({ ClientID: response[i].ClientID, FirstName: response[i].FirstName, LastName: response[i].LastName, DOB: formatDate(response[i].DOB), Gender: response[i].Gender, FullStreetAddress: response[i].FullStreetAddress, City: response[i].City, State: response[i].State, Email_Address: response[i].Email_Address });                                                                        
